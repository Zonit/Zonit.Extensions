...
/// <code>
/// var warsaw   = new Zone("Europe/Warsaw");   // IANA — DST-aware
/// var fixedUtc = new Zone(0);                 // UTC
/// var offset   = new Zone(2);                 // UTC+02:00, no DST
/// Zone parsed = "America/New_York";           // implicit from string
/// Zone fromHours = 4;                          // implicit from int hours
/// var nowInZone = warsaw.ConvertFromUtc(DateTime.UtcNow);
/// </code>
///
/// <para><b>Conversions.</b> Implicit <see cref="string"/> ⇄ <see cref="Zone"/> and
/// <see cref="int"/> → <see cref="Zone"/> mirror the way <see cref="Culture"/> works,
...
/// <see cref="Zone"/>s representing the same offset but built differently
/// (e.g. <c>new Zone(0)</c> vs <c>new Zone("UTC")</c>) are equal — the
/// constructor normalises both to <c>"UTC"</c>.</para>
///
/// <para><b>Naming.</b> This type is the RENAME of the former <c>Zonit.Extensions.TimeZone</c>,
/// which collided with the long-deprecated <c>System.TimeZone</c> (replaced by
/// <see cref="System.TimeZoneInfo"/> in .NET Framework 4.6+): <c>ImplicitUsings</c> pulls in
/// <c>using System;</c>, so every consumer naming the old type unqualified got CS0104.
/// <c>Zone</c> itself does not collide with anything in <c>System</c>. Member names were
/// deliberately left alone — <c>ICultureState.TimeZone</c>, <c>ICultureManager.SetTimeZone</c>
/// and <c>CultureOption.DefaultTimeZone</c> are unambiguous and describe the concept.</para>
/// </remarks>
[TypeConverter(typeof(ValueObjectTypeConverter<Zone>))]
[JsonConverter(typeof(TimeZoneJsonConverter))]
public readonly struct Zone : IEquatable<Zone>, IParsable<Zone>
{
    /// <summary>Empty / unset time zone (<c>default(Zone)</c>).</summary>
    public static readonly Zone Empty = default;

    /// <summary>Coordinated Universal Time. Equivalent to <c>new Zone("UTC")</c>.</summary>
    public static readonly Zone Utc = new("UTC", TimeSpan.Zero, isFixedOffset: true);

    /// <summary>
    /// Maximum canonical-id length. Set to 64 — comfortably above the longest IANA
    /// id (<c>"America/Argentina/ComodRivadavia"</c>, 32 chars) and the longest Windows
    /// id (<c>"Central European Standard Time"</c>, 30 chars), with headroom for any
    /// future additions. Used by EF Core / model-binding layers to size the storage column.
    /// </summary>
    ...
    public static Zone Local => new(BclTimeZoneInfo.Local.Id, isFixedOffset: false);
    ...
    private Zone(string id, bool isFixedOffset)
    {
        _value = id;
    ...
    private Zone(string normalisedId, TimeSpan offset, bool isFixedOffset)
    {
        _value = normalisedId;
        _offset = offset;
        _isFixedOffset = isFixedOffset;
    ...
    public Zone(int hours) : this(TimeSpan.FromHours(ValidateHours(hours)))
    {
    }
    ...
    /// <summary>
    /// Resolves this <see cref="Zone"/> into a runtime <see cref="BclTimeZoneInfo"/>.
    ...
    public static implicit operator string(Zone tz) => tz.Value;
    ...
    /// </summary>
    public static implicit operator Zone(string? value)
    {
        if (string.IsNullOrWhiteSpace(value))
            return Empty;
    ...
    public static implicit operator Zone(int hours) => new(hours);

    /// <summary>Implicit conversion from <see cref="TimeSpan"/> offset.</summary>
    public static implicit operator Zone(TimeSpan offset) => new(offset);
    ...
    /// <summary>
    /// Attempts to build a <see cref="Zone"/> from <paramref name="value"/>. Returns
    ...
    /// </summary>
    public static bool TryCreate(string? value, out Zone timeZone)
    {
        if (string.IsNullOrWhiteSpace(value))
        {
            timeZone = Empty;
        ...
        try
        {
            ...
            timeZone = new Zone(info.Id, isFixedOffset: false);
            return true;
        }
        catch (TimeZoneNotFoundException)
        {
    ...
    /// <inheritdoc />
    public static Zone Parse(string s, IFormatProvider? provider)
        => TryParse(s, provider, out var r) ? r : throw new FormatException($"'{s}' is not a valid Zone.");

    /// <inheritdoc />
    public static bool TryParse([NotNullWhen(true)] string? s, IFormatProvider? provider, out Zone result)
...
/// <summary>
/// AOT-safe JSON converter for <see cref="Zone"/>. Reads any string the
/// <see cref="Zone.TryCreate(string?, out Zone)"/> grammar accepts; writes the
/// canonical <see cref="Zone.Value"/>.
/// </summary>
public sealed class TimeZoneJsonConverter : JsonConverter<Zone>
{
    /// <inheritdoc />
    public override Zone Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        var value = reader.GetString();
        return Zone.TryCreate(value, out var tz) ? tz : Zone.Empty;
    }
