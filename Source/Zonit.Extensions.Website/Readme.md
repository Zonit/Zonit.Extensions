...
**`AddWebsite()` does not load configuration.** If you use
[Zonit.Extensions.Configuration](../Zonit.Extensions.Configuration/Readme.md), call
`builder.AddAppData()` yourself and put it **above** `AddWebsite`. That order is a requirement:
`o.AddArea<T>()` runs each area's `ConfigureServices` *during* `AddWebsite`, and areas read
configuration there — a plugin resolving its connection string, for one. Anything `AddWebsite` did
with configuration would already be too late for the areas it just registered, so the kernel does
not try.
